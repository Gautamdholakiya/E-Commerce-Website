...
export const cartSlices = createSlice({
  ...
  reducers: {
    addToCart: (state, action) => {
      ...
      const index = state.cart.findIndex((item) => item.key === curentItem.key);

      if (index === -1) {
        state.cart.push({ ...curentItem, quantity: 1 });
      } else {
...
export const { addToCart, removeToCart, resetCart } = cartSlices.actions;
